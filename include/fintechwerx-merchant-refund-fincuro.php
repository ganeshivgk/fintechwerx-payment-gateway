...
// require_once 'user-refund-process.php';
// require_once 'merchantdashboard.php';
...
/* if (file_exists(plugin_dir_path(__FILE__) . 'access-token.php')) {
    require_once plugin_dir_path(__FILE__) . 'access-token.php';
} else {
    echo 'The file access-token.php does not exist in the directory';
} */
...
function my_woocommerce_my_admin_refunds() {
    ...
    $user_id = get_current_user_id();
     global $fintech_base_url;
            ...
            // Get or regenerate the access token
 /*        $access_token = get_transient('my_admin_refunds_access_token');
        if (!$access_token) {
            $access_token = get_access_token();
            set_transient('my_admin_refunds_access_token', $access_token, 300);
        }

        // Check if the access token is more than 300 seconds old and regenerate if necessary
        $access_token_age = get_transient('my_admin_refunds_access_token_age');
        if (!$access_token_age) {
            $access_token_age = time();
            set_transient('my_admin_refunds_access_token_age', $access_token_age);
        }
        if (time() - $access_token_age > 300) {
            $access_token = get_access_token();
            set_transient('my_admin_refunds_access_token', $access_token, 300);
            set_transient('my_admin_refunds_access_token_age', time());
        } */
